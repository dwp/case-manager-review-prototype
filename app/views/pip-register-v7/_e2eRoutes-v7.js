...
// for yourself or someone else
 router.post(`/${folderForViews}/signposting-eligibility/for-yourself-or-someone-else`, function (request, response) {
    ...
    if (claimingSelf === 'myself') {
      ...
      response.redirect('/pip-register-v7/signposting-eligibility/national-insurance-number');
    } else if (claimingSelf === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/applying-for-someone-else');
    } else {
      response.redirect('/pip-register-v7/signposting-eligibility/for-yourself-or-someone-else');
    }
  })

// someone else
 router.post(`/${folderForViews}/signposting-eligibility/applying-for-someone-else`, function (request, response) {
    const applysomeoneelse = request.body['claiming-self'];
    if (applysomeoneelse === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v7/signposting-eligibility/third-party-with-applicant');
    } else if (applysomeoneelse === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/do-you-live-in-wales');
      } else if (applysomeoneelse === 'someone-else-2') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/collect-more-details');
    } else {
      response.redirect('/pip-register-v7/signposting-eligibility/applying-for-someone-else');
    }
  })
...
// do you live in wales
 router.post(`/${folderForViews}/signposting-eligibility/do-you-live-in-wales`, function (request, response) {
    const claimingSelf = request.body['claiming-self'];
    if (claimingSelf === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v7/signposting-eligibility/appointee-welsh-communication');
    } else if (claimingSelf === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/answer-for-the-applicant');
    } else {
      response.redirect('/pip-register-v7/signposting-eligibility/appointee-welsh-communication');
    }
  })
...
//welsh communication
  router.post(`/${folderForViews}/signposting-eligibility/appointee-welsh-communication`, function (request, response) {
    var welshcommunication = request.session.data['welshcommunication']
    if (welshcommunication == 'no') {
      const redirectPath = `/${folderForViews}/signposting-eligibility/answer-for-the-applicant`;
validatePath(response, redirectPath);
    } else if (welshcommunication == "yes") {
      const redirectPath = `/${folderForViews}/signposting-eligibility/collect-more-details`;
validatePath(response, redirectPath);
    }
  })

// Name and DOB
router.post(`/${folderForViews}/signposting-eligibility/appointee-name`, function (request, response) {
  const firstName = request.session.data['first-name'];
  const lastName = request.session.data['last-name'];

  const redirectPath = `/${folderForViews}/signposting-eligibility/appointee-type`;
validatePath(response, redirectPath);;
})

// Name and DOB
router.post(`/${folderForViews}/signposting-eligibility/appointee-type`, function (request, response) {
  const firstName = request.session.data['first-name'];
  const lastName = request.session.data['last-name'];

  const redirectPath = `/${folderForViews}/signposting-eligibility/check-personal-details-appointee`;
validatePath(response, redirectPath);;
})


// Check personal details appointee
router.post(`/${folderForViews}/signposting-eligibility/check-personal-details-appointee`, function (request, response) {
  const firstName = request.session.data['first-name'];
  const lastName = request.session.data['last-name'];
...
// someone else third party
 router.post(`/${folderForViews}/signposting-eligibility/applying-for-someone-else`, function (request, response) {
    ...
    if (applysomeoneelse === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v6/signposting-eligibility/third-party-with-applicant');
    } else if (applysomeoneelse === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v6/signposting-eligibility/collect-more-details');
    } else {
      response.redirect('/pip-register-v6/signposting-eligibility/collect-more-details');
    }
  })
  ...
  //  third party with applicant
 router.post(`/${folderForViews}/signposting-eligibility/third-party-with-applicant`, function (request, response) {
    const applysomeoneelse = request.body['claiming-self'];
    if (applysomeoneelse === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v6/signposting-eligibility/third-party-name');
    } else if (applysomeoneelse === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v6/signposting-eligibility/special-rules-for-end-of-life-application-third-party');
    } else {
      response.redirect('/pip-register-v6/signposting-eligibility/third-party-with-applicant');
    }
  })
  ...
  // third party name
router.post(`/${folderForViews}/signposting-eligibility/third-party-name`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/i-need-to-speak-to-the-applicant`;
validatePath(response, redirectPath);;
})

  // i-need-to-speak-to-the-applicant
router.post(`/${folderForViews}/signposting-eligibility/i-need-to-speak-to-the-applicant`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/consent-for-third-party-to-answer-questions`;
validatePath(response, redirectPath);;
})


// consent-for-third-party-to-answer-questions
 router.post(`/${folderForViews}/signposting-eligibility/consent-for-third-party-to-answer-questions`, function (request, response) {
    const claimingSelf = request.body['claiming-self'];
    if (claimingSelf === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v6/signposting-eligibility/stay-with-third-party-until-call-ends');
    } else if (claimingSelf === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v6/signposting-eligibility/consent-for-third-party-or-answer-questions-for-yourself');
    } else {
      response.redirect('/pip-register-v6/signposting-eligibility/stay-with-third-party-until-call-ends');
    }
  })

  // stay-with-third-party-until-call-ends
router.post(`/${folderForViews}/signposting-eligibility/stay-with-third-party-until-call-ends`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/answer-for-the-applicant`;
validatePath(response, redirectPath);;
})

  // answer for the applicant
router.post(`/${folderForViews}/signposting-eligibility/answer-for-the-applicant`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/national-insurance-number`;
validatePath(response, redirectPath);;
})
  ...
  // SREL-third party
   router.post(`/${folderForViews}/signposting-eligibility/special-rules-for-end-of-life-application-third-party`, function (request, response) {
    var srel = request.session.data['special-rules-for-end-of-life-application-third-party']
    if (srel == 'no') {
      const redirectPath = `/${folderForViews}/signposting-eligibility/third-party-cannot-continue-application`;
validatePath(response, redirectPath);
    } else if (srel == "yes") {
...
validatePath(response, redirectPath);
  ...
  // third party - pass-phone-to-applicant-for-security
router.post(`/${folderForViews}/signposting-eligibility/pass-phone-to-applicant-for-security`, function (request, response) {
  const firstName = request.session.data['first-name'];
  const lastName = request.session.data['last-name'];
...
validatePath(response, redirectPath);;
 ...
 // third party pass-phone-to-third-party-to-start-application
    router.post(`/${folderForViews}/signposting-eligibility/pass-phone-to-third-party-to-start-application`, function (request, response) {
     const redirectPath = `/${folderForViews}/task-list`;
validatePath(response, redirectPath);
  })

  router.post(`/${folderForViews}/healthcare-professional/pass-phone-to-applicant-for-consent`, function (request, response) {
      const redirectPath = `/${folderForViews}/healthcare-professional/consent-2`;
validatePath(response, redirectPath);
  })

 // third party pass-phone-to-third-party-to-complete-application
    router.post(`/${folderForViews}/healthcare-professional/pass-phone-to-third-party-to-complete-application`, function (request, response) {
...
//third party flows end - some third party routes are still below this line


  //NEW CORE PROTOTYPE JOURNEY ROUTES - 10 MAY 2026

  // updated for v7 appointees - 17 JUL 2026
...
// for yourself or someone else
 router.post(`/${folderForViews}/signposting-eligibility/for-yourself-or-someone-else`, function (request, response) {
    ...
    if (claimingSelf === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v6/signposting-eligibility/national-insurance-number');
    } else if (claimingSelf === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v6/signposting-eligibility/applying-for-someone-else');
    } else {
      response.redirect('/pip-register-v6/signposting-eligibility/for-yourself-or-someone-else');
    }
  })
...
validatePath(response, redirectPath);
  ...
  router.post(`/${folderForViews}/signposting-eligibility/claiming-dla`, function (request, response) {
    ...
    if (claimingdla == "yes") {
        const redirectPath = `/${folderForViews}/signposting-eligibility/dla-invitation-letter`;
validatePath(response, redirectPath);
    }
...
validatePath(response, redirectPath);
 ...
 //DLA JOURNEY SCREENS BEGIN//


  //cdla-invitation-letter
  router.post(`/${folderForViews}/signposting-eligibility/dla-invitation-letter`, function (request, response) {
    var claimingdla = request.session.data['claimingdla']
    if (claimingdla == "yes") {
        const redirectPath = `/${folderForViews}/signposting-eligibility/continue-application`;
validatePath(response, redirectPath);
    }
    else if (claimingdla == "no") {
        const redirectPath = `/${folderForViews}/signposting-eligibility/health-condition-improved`;
validatePath(response, redirectPath);
    }
  })

    // continue-application
router.post(`/${folderForViews}/signposting-eligibility/continue-application`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/appointee-check`;
validatePath(response, redirectPath);;
})
...
// health-condition-improved
 router.post(`/${folderForViews}/signposting-eligibility/health-condition-improved`, function (request, response) {
    const applysomeoneelse = request.body['claiming-self'];
    if (applysomeoneelse === 'myself') {
      request.session.data['journey'] = 'core';
      response.redirect('/pip-register-v7/signposting-eligibility/need-to-apply-for-PIP');
    } else if (applysomeoneelse === 'someone-else') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/apply-today');
      } else if (applysomeoneelse === 'got-worse') {
      request.session.data['journey'] = '3rd';
      response.redirect('/pip-register-v7/signposting-eligibility/dla-highest-rate');
    } else {
      response.redirect('/pip-register-v7/signposting-eligibility/health-condition-improved');
    }
  })


    //need to apply for pip continue-application
router.post(`/${folderForViews}/signposting-eligibility/need-to-apply-for-PIP`, function (request, response) {
  const redirectPath = `/${folderForViews}/signposting-eligibility/security-questions`;
validatePath(response, redirectPath);;
})
...
//apply-today
  router.post(`/${folderForViews}/signposting-eligibility/apply-today`, function (request, response) {
    var welshcommunication = request.session.data['welshcommunication']
    if (welshcommunication == 'no') {
      const redirectPath = `/${folderForViews}/signposting-eligibility/end-call`;
validatePath(response, redirectPath);
    } else if (welshcommunication == "yes") {
      const redirectPath = `/${folderForViews}/signposting-eligibility/security-questions`;
validatePath(response, redirectPath);
...
//dla-highest-rate
  router.post(`/${folderForViews}/signposting-eligibility/dla-highest-rate`, function (request, response) {
    var welshcommunication = request.session.data['welshcommunication']
    if (welshcommunication == 'no') {
      const redirectPath = `/${folderForViews}/signposting-eligibility/need-to-apply-for-PIP`;
validatePath(response, redirectPath);
    } else if (welshcommunication == "yes") {
      const redirectPath = `/${folderForViews}/signposting-eligibility/do-not-need-to-apply-for-pip`;
validatePath(response, redirectPath);
    }
  })

  //dla-medical-evidence
  router.post(`/${folderForViews}/healthcare-professional/dla-medical-evidence`, function (request, response) {
    var decision = request.session.data['decision']
    if (decision == 'no') {
      const redirectPath = `/${folderForViews}/healthcare-professional/pass-phone-to-third-party-to-complete-application`;
validatePath(response, redirectPath);
...
//appointee details v7

  router.post(`/${folderForViews}/appointee-details/questions-about-the-appointee`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/what-is-your-postcode`;
validatePath(response, redirectPath)
  })

  router.post(`/${folderForViews}/appointee-details/what-is-your-postcode`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/select-your-address`;
validatePath(response, redirectPath)
  })

 // Select your address page
  router.post(`/${folderForViews}/appointee-details/select-your-address`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/correspondence-address`;
validatePath(response, redirectPath);
  })


  // Enter address manually page
  router.post(`/${folderForViews}/appointee-details/enter-address-manually-country`, function (_request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/correspondence-address`;
validatePath(response, redirectPath);
  })


 // Is this the address we should use to contact you
  router.post(`/${folderForViews}/appointee-details/correspondence-address`, function (request, response) {
    var sendLettersElsewhere = request.session.data['should-we-write-to-you']
    if (sendLettersElsewhere == 'yes') {
      const redirectPath = `/${folderForViews}/appointee-details/alt-formats/written-format`;
validatePath(response, redirectPath);
    } else if (sendLettersElsewhere == 'no') {
      const redirectPath = `/${folderForViews}/appointee-details/correspondence-postcode`;
validatePath(response, redirectPath);
...
validatePath(response, redirectPath);
  ...
  router.post(`/${folderForViews}/appointee-details/correspondence-postcode`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/select-your-address-correspondence`;
validatePath(response, redirectPath);
  })
    //select correspondence address
        router.post(`/${folderForViews}/appointee-details/select-your-address-correspondence`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/written-format`;
validatePath(response, redirectPath);
  })

// Confirm correspondence address manually page
  router.post(`/${folderForViews}/appointee-details/correspondence-enter-address-manually`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/written-format`;
validatePath(response, redirectPath);
  })

 // Would you like us to send your letters in another way, like larger text, audio or braille?
router.post(`/${folderForViews}/appointee-details/alt-formats/written-format`, function (request, response) {
  ...
  if (writtenFormat === 'Standard letter (12-point Arial text on white paper)') {
    const redirectPath = `/${folderForViews}/appointee-details/mobile-phone-number`;
validatePath(response, redirectPath);;
  } else if (writtenFormat === 'Letter with changes to colour or print size') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/large-print`;
validatePath(response, redirectPath);;
  } else if (writtenFormat === 'Audio') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-type-of-audio-format`;
validatePath(response, redirectPath);;
  } else if (writtenFormat === 'Braille') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-type-of-braille-do-you-need`;
validatePath(response, redirectPath);;
  } else if (writtenFormat === 'British Sign Language video') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-video-format-do-you-need`;
validatePath(response, redirectPath);;
  } else if (writtenFormat === 'Email') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/email-reason-1`;
validatePath(response, redirectPath);;
  } else {
    // Fallback: no option selected
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/written-format`;
...
router.post(`/${folderForViews}/appointee-details/alt-formats/large-print`, function (request, response) {
  ...
  if (largePrint == 'coloured-paper'|| largePrint == 'coloured-paper,large-print') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-colour-paper-do-you-need`;
validatePath(response, redirectPath);

  } else if (largePrint == 'large-print') {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-size-print-do-you-need`;
validatePath(response, redirectPath);

 } else if (largePrint == 'coloured-paper,large-print') {
    const redirectPath = `/${folderForViews}/appointee-details/what-colour-paper-do-you-need`;
validatePath(response, redirectPath);
...
router.post(`/${folderForViews}/appointee-details/alt-formats/what-colour-paper-do-you-need`, function (request, response) { 
    var largePrint = request.session.data['largePrint'];
 ...
 //What size print u need 
  router.post(`/${folderForViews}/appointee-details/alt-formats/what-size-print-do-you-need`, function (request, response) {
...
validatePath(response, redirectPath);;
 ...
 // What is your mobile number?
   router.post(`/${folderForViews}/appointee-details/mobile-phone-number`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/want-to-tell-us-another-phone-number`;
validatePath(response, redirectPath);
  })


 // What is your other number?
   router.post(`/${folderForViews}/appointee-details/other-phone-number`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/check-contact-information`;
validatePath(response, redirectPath);
  })

  
router.post(`/${folderForViews}/appointee-details/want-to-tell-us-another-phone-number`, function (request, response) {
    var otherNumberappointeeYesNo = request.session.data['otherNumberappointeeYesNo']
    if (otherNumberappointeeYesNo == "yes") {
        const redirectPath = `/${folderForViews}/appointee-details/other-phone-number`;
validatePath(response, redirectPath);
    }
    else if (otherNumberappointeeYesNo == "no") {
        const redirectPath = `/${folderForViews}/appointee-details/check-contact-information`;
validatePath(response, redirectPath);
    }
  })
...
validatePath(response, redirectPath);
  ...
  // Would you like us to send your letters in another way, like larger text, audio or braille?
router.post(`/${folderForViews}/appointee-details/alt-formats/what-type-of-audio-format`, function (request, response) {
...
validatePath(response, redirectPath);;
    ...
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-is-your-email`;
validatePath(response, redirectPath);;
...
validatePath(response, redirectPath);;
    ...
    // Fallback: no option selected
    const redirectPath = `./${folderForViews}/appointee-details/what-type-of-audio-format`;
validatePath(response, redirectPath);;
  }
})

// Do you want to receive text updates
  router.post(`/${folderForViews}/appointee-details/do-you-want-to-receive-text-updates`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/mobile-phone-number`;
validatePath(response, redirectPath);
  })

 router.post(`/${folderForViews}/appointee-details/alt-formats/what-is-your-email`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/mobile-phone-number`;
validatePath(response, redirectPath);
  })
  ...
  router.post(`/${folderForViews}/appointee-details/alt-formats/what-type-of-braille-do-you-need`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/mobile-phone-number`;
validatePath(response, redirectPath);
  })

  // video preferences 
router.post(`/${folderForViews}/appointee-details/alt-formats/what-video-format-do-you-need`, function (request, response) {
...
validatePath(response, redirectPath);;
   ...
   router.post(`/${folderForViews}/appointee-details/alt-formats/email-reason-1`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/email-reason`;
validatePath(response, redirectPath);
  })

// When we send you an email, do you need us to attach the letter as a Word document?
  router.post(`/${folderForViews}/appointee-details/alt-formats/email-reason`, function (request, response) {
    const redirectPath = `/${folderForViews}/appointee-details/alt-formats/what-is-your-email`;
validatePath(response, redirectPath);
  })
...
//appointee - applicant details v7 begin
 ...
 router.post(`/${folderForViews}/contact-details-applicant/your-postcode-building-number`, function (request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/confirm-address`;
validatePath(response, redirectPath)
  })


 router.post(`/${folderForViews}/contact-details-applicant/confirm-address`, function (request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/correspondence-address`;
validatePath(response, redirectPath)
  })


// Is this the address we should use to contact you
  router.post(`/${folderForViews}/contact-details-applicant/correspondence-address`, function (request, response) {
    var sendLettersElsewhere = request.session.data['should-we-write-to-you']
    if (sendLettersElsewhere == 'yes') {
      const redirectPath = `/${folderForViews}/contact-details-applicant/mobile-phone-number`;
validatePath(response, redirectPath);
    } else if (sendLettersElsewhere == 'no') {
      const redirectPath = `/${folderForViews}/contact-details-applicant/correspondence-postcode`;
validatePath(response, redirectPath);
    } else if (sendLettersElsewhere == 'No, I dont have a home address') {
      const redirectPath = `/${folderForViews}/contact-details-applicant/mobile-phone-number`;
validatePath(response, redirectPath);
 ...
 //correspondence-postcode
  router.post(`/${folderForViews}/contact-details-applicant/correspondence-postcode`, function (request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/select-your-address-correspondence`;
validatePath(response, redirectPath);
  })

   router.post(`/${folderForViews}/contact-details-applicant/select-your-address-correspondence`, function (request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/mobile-phone-number`;
validatePath(response, redirectPath);
  })

// Enter address manually page
  router.post(`/${folderForViews}/contact-details-applicant/correspondence-enter-address-manually`, function (_request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/mobile-phone-number`;
validatePath(response, redirectPath);
  })

 router.post(`/${folderForViews}/contact-details-applicant/mobile-phone-number`, function (request, response) {
    const redirectPath = `/${folderForViews}/contact-details-applicant/want-to-tell-us-another-phone-number`;
validatePath(response, redirectPath)
  })



router.post(`/${folderForViews}/contact-details-applicant/want-to-tell-us-another-phone-number`, function (request, response) {
    var otherNumberappointeeYesNo = request.session.data['otherNumberappointeeYesNo']
    if (otherNumberappointeeYesNo == "yes") {
        const redirectPath = `/${folderForViews}/contact-details-applicant/other-phone-number`;
validatePath(response, redirectPath);
    }
    else if (otherNumberappointeeYesNo == "no") {
        const redirectPath = `/${folderForViews}/contact-details-applicant/check-contact-information`;
validatePath(response, redirectPath);
    }
  })
...
// check contact information 2
  router.post(`/${folderForViews}/contact-details-applicant/check-contact-information-2`, function (request, response) {
    const redirectPath = `/${folderForViews}/nationality/start`;
validatePath(response, redirectPath);
  })

  // check contact information 3
  router.post(`/${folderForViews}/contact-details-applicant/check-contact-information-3`, function (request, response) {
    const redirectPath = `/${folderForViews}/nationality/start`;
validatePath(response, redirectPath);
  })

  // check contact information 4
  router.post(`/${folderForViews}/contact-details-applicant/check-contact-information-4`, function (request, response) {
    const redirectPath = `/${folderForViews}/nationality/start`;
validatePath(response, redirectPath);
  })

//paper form only
router.post(`/${folderForViews}/what-happens-next/appointee-paper-form`, function (request, response) {
    const redirectPath = `/${folderForViews}/what-happens-next/paper-whn-2`;
validatePath(response, redirectPath)
  })


//appointee - applicant details v7 end
