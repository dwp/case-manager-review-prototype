...
const allowedFolders = new Set(['pip-register-v3']);
function ensureSafeRedirectPath(redirectPath) {
  const hasScheme = /^[a-zA-Z][a-zA-Z0-9+.-]*./.test(redirectPath); 
  if (hasScheme || redirectPath.startsWith('//')) {
    throw new Error('Unsafe redirect path');
  }
  const normalizedPath = path.posix.normalize('/' + redirectPath.replace(/^\.\//, '').replace(/^\/+/, ''));
  const firstSegment = normalizedPath.split('/')[1]||'';
  if (!allowedFolders.has(firstSegment)) {
    throw new Error('Redirect path not in allowed folders');
  }
...
function validatePath(res, redirectPath) {
    ...
    if (!allowedFolders.includes(folderForViews)) {
      console.log("FOLDER NOT ALLOWED");
      const error = new Error('Client Error - Folder not found on allowed folder list');
